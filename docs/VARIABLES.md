...
| `mneme_prepare_backup`        | bool     | `false`                   | If true, backup will be prepared before archiving. It will reduce significally time of recovery process but will affect server CPU each time it creates backup. It's also incompatible with incremental backup strategy. |
...
| **Variable**                                           | **Type** | **Default**    | **Description**                                                                                                                                                                                                                          |
|:-------------------------------------------------------|:---------|:---------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `mneme_safe_slave_backup`                        | bool     | `false`        | If true, mariabackup will stop the replication SQL thread and waits to start backing up until Slave_open_temp_tables in SHOW STATUS is zero. It will wait until mneme_safe_slave_backup_timeout.                                   |
...
| `mneme_safe_slave_autorestore_replication`       | bool     | `false`        | Force the replication start if something happend during the script execution                                                                                                                                                             |
